...
export const PRICES_BY_BLOCK = (tokenAddress: Address, blocks: Block[]) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block) => `
...
export const HOURLY_PAIR_RATES = (pairAddress: Address, blocks: Block[]) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block) => `
...
export const SHARE_VALUE = (pairAddress: Address, blocks: Block[]) => {
  let queryString = 'query blocks {';
  queryString += blocks.map(
    (block) => `
      t${block.timestamp}:pair(id:"${pairAddress}", block: { number: ${block.number} }) {
...
export const USER = (blockNumber: number, account: Address) => {
  const queryString = `
    query users {
      user(id: "${account}", block: {number: ${blockNumber}}) {
        liquidityPositions
      }
    }
`;
...
export const PAIR_DAY_DATA_BULK = (pairs: Address[], startTimestamp: number) => {
  let pairsString = `[`;
  pairs.map((pair) => {
...
export const MINING_POSITIONS = (account: Address) => {
  const queryString = `
    query users {
      user(id: "${account}") {
        miningPosition {
...
export const PAIRS_HISTORICAL_BULK = (blockNumber: number, pairs: Address[]) => {
  let pairsString = `[`;
  pairs.map((pair) => {
    return (pairsString += `"${pair}"`);
  });
  ...
  const queryString = `
  query pairs {
    pairs(first: 200, where: {id_in: ${pairsString}}, block: {number: ${blockNumber}}, orderBy: trackedReserveETH, orderDirection: desc) {
      id
      reserveUSD
      trackedReserveETH
      volumeUSD
