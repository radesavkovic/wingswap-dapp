...
import { Currency, Percent, Router, SwapParameters, Trade, TradeType } from '@wingsswap/sdk';
...
export function swapErrorToUserReadableMessage(error: any): string {
  ...
  switch (reason) {
    ...
    case 'WingswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT':
    case 'WingswapV2Router: EXCESSIVE_INPUT_AMOUNT':
    ...
    case 'WingswapV2: TRANSFER_FAILED':
      return `The output token cannot be transferred. There may be an issue with the output token.`;
    case 'WingswapV2: K':
      return `The Wingswap invariant x*y=k was not satisfied by the swap. This usually means one of the tokens you are swapping incorporates custom behavior on transfer.`;
    ...
    default:
      if (reason?.indexOf('undefined is not an object') !== -1) {
        ...
        return `An error occurred when trying to execute this swap. You may need to increase your slippage tolerance. If that does not work, there may be an incompatibility with the token you are trading. Note fee on transfer and rebase tokens are incompatible with Wingswap V3.`;
      }
